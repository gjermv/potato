...
def createPage(document, dict, image_folder, tunnelinfo={'tunnel_name': 'Eksempeltunnel'}):
    ...
    body_cells[1].text = str(dict['Pel'])
    ...
    body_cells[0].text = dict['Registrering']
...
def TimeStampToExcel(folder):
    print('Side;Pel;Skade;Kommentar;Bilde')
    
    for image in glob.glob(folder+'\\*.jpg'):
        
        img = Image.open(image)
        exif_data = img._getexif()
        # print(image)                                                           ### For feilsøking ved 
        # print(exif_data)
        ...
        try:
            ...
            txt = txt.replace('\n', ' ').replace('Ã¸','ø').replace('Ã¥','å').replace('Ã','Ø')
            txt = txt.replace(' pel', '')
            txt_tab = txt.split(' ',2)
            txt_skade = txt_tab[2].split('-')
            
        
            try:
                print(txt_tab[0].lower(),txt_tab[1],txt_skade[0],txt_skade[1],os.path.basename(image).split('.')[0],sep=';')
            except: 
                print(txt_tab[0].lower(),txt_tab[1],'-Info-',txt_skade[0],os.path.basename(image).split('.')[0],sep=';')
                
        except: 
            print('-;-;-;Ingen info;{}'.format(os.path.basename(image).split('.')[0]))
